...
import { recalculateProjectMetrics } from '../../utils/simulation/projectGenerator'
import { projectDB } from '../../utils/simulation/database'

function TaskTimeline({ projectPlan, startDate, onTaskUpdate, onSimulatedDaysChange, projectData }) {
  ...
  const [showHistory, setShowHistory] = useState(null) // taskId-itemName
  const [initializedBase, setInitializedBase] = useState(false)

  // Ensure each task keeps its baseline duration for what-if adjustments
  useEffect(() => {
    if (initializedBase) return
    const tasksWithBase = projectPlan.tasks.map((task) => ({
      ...task,
      baseDuration: task.baseDuration ?? task.actualDuration,
      whatIfEnabled: task.whatIfEnabled || false,
      whatIfExtraDays: task.whatIfExtraDays || 0
    }))
    const totalDuration = tasksWithBase.reduce((sum, t) => sum + (t.actualDuration || 0), 0)
    const totalDurationMonths = tasksWithBase.reduce((sum, t) => sum + (t.actualDurationMonths || Math.ceil((t.actualDuration || 0) / 30)), 0)
    onTaskUpdate({ ...projectPlan, tasks: tasksWithBase, totalDuration, totalDurationMonths })
    setInitializedBase(true)
  }, [initializedBase, onTaskUpdate, projectPlan])

  const recomputeSchedule = (tasks) => {
    let rollingDay = 0
    let rollingMonth = 0

    const scheduled = tasks.map((task) => {
      const actualDuration = Math.max(1, task.actualDuration || 1)
      const actualDurationMonths = Math.max(1, Math.ceil(actualDuration / 30))
      const updated = {
        ...task,
        actualDuration,
      ...
      rollingDay += actualDuration
      rollingMonth += actualDurationMonths
      return updated
    })

    const totalDuration = scheduled.reduce((sum, t) => sum + t.actualDuration, 0)
    const totalDurationMonths = scheduled.reduce((sum, t) => sum + t.actualDurationMonths, 0)
    return { scheduled, totalDuration, totalDurationMonths }
  }

  const handleWhatIfChange = (taskId, enabled, extraDays) => {
    const updatedTasks = projectPlan.tasks.map((task) => {
      if (task.id !== taskId) return task
      const base = task.baseDuration ?? task.actualDuration
      const nextDuration = enabled ? Math.max(1, base + extraDays) : base
      return {
        ...task,
        baseDuration: base,
        whatIfEnabled: enabled,
        whatIfExtraDays: enabled ? extraDays : 0,
        actualDuration: nextDuration
      }
    })

    const { scheduled, totalDuration, totalDurationMonths } = recomputeSchedule(updatedTasks)
    const updatedPlan = {
      ...projectPlan,
      tasks: scheduled,
      totalDuration,
    ...
    const recalculated = recalculateProjectMetrics(
      updatedPlan,
      originalCost,
      simulatedDaysElapsed
    )
    onTaskUpdate(recalculated)
  }
  ...
  const handleTaskComplete = (taskId) => {
    const updatedTasks = projectPlan.tasks.map(task => 
      task.id === taskId 
        ? { ...task, isCompleted: !task.isCompleted, status: !task.isCompleted ? 'completed' : 'not-started', completionPercentage: !task.isCompleted ? 100 : 0 }
        : task
    )
  ...
  const handleSubActivityToggle = (taskId, subActivityId) => {
    const updatedTasks = projectPlan.tasks.map(task => {
  ...
  const handleWorkItemUpdate = (taskId, itemName, field, newValue) => {
    const updatedTasks = projectPlan.tasks.map(task => {
  ...
  const handleSaveWorkItem = async (taskId, itemName) => {
  ...
  const getTaskDateRange = (task) => {
    // Use startDate from projectData if available, otherwise fall back to prop
    const baseStart = projectData?.startDate || startDate
  ...
  const totalMonthsRounded = Math.ceil((projectPlan.totalDuration || 0) / 30)
  ...
  return (
    <div className="card task-timeline">
      ...
      <div className="duration-summary">
        <span>Total Duration: {projectPlan.totalDuration || 0} days</span>
        <span>|</span>
        <span>{totalMonthsRounded} months</span>
      </div>
              ...
              <div className="task-content">
                ...
                <div className="what-if-row">
                  <label className="what-if-label">
                    <input
                      type="checkbox"
                      checked={task.whatIfEnabled || false}
                      onChange={(e) => handleWhatIfChange(task.id, e.target.checked, task.whatIfExtraDays || 0)}
                    />
                    What-if simulation
                  </label>
                  <div className="what-if-inputs">
                    <span className="detail-label">Extra days:</span>
                    <input
                      type="number"
                      min="0"
                      value={task.whatIfExtraDays || 0}
                      onChange={(e) => handleWhatIfChange(task.id, task.whatIfEnabled || false, parseInt(e.target.value) || 0)}
                      className="what-if-input"
                      disabled={!task.whatIfEnabled}
                    />
                    <span className="what-if-hint">Total updates immediately</span>
                  </div>
                </div>
