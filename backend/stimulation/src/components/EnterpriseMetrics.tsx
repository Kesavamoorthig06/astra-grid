...
import type { ProjectPlan } from '../utils/projectGenerator'
...
function EnterpriseMetrics({ projectPlan, startDate, simulatedDaysElapsed }: EnterpriseMetricsProps) {
  const criticalPath: CriticalPathTask[] = useMemo(
  ...
  const criticalTasks = criticalPath.filter((t: CriticalPathTask) => t.isCritical)
  const criticalPathDuration = Math.max(...criticalPath.map((t: CriticalPathTask) => t.earlyFinish))
      ...
      <div className="metrics-grid">
        <div className="metric-card critical-path-card">
          ...
          <div className="critical-tasks-list">
            {criticalTasks.map((task: CriticalPathTask, idx: number) => (
      ...
      <div className="metric-card cashflow-card">
        ...
        <div className="cashflow-table">
          <table>
            ...
            <tbody>
              {cashFlow.slice(0, 6).map((cf: CashFlowForecast, idx: number) => (
                <tr key={idx}>
