...
import type { Task, ProjectPlan } from './projectGenerator'
...
export function calculateCriticalPath(tasks: Task[]): CriticalPathTask[] {
  const criticalTasks: CriticalPathTask[] = tasks.map(task => ({
    ...task,
    earlyStart: 0,
    earlyFinish: 0,
    lateStart: 0,
...
export function generateCashFlowForecast(
  ...
  const monthlyForecasts: CashFlowForecast[] = []
